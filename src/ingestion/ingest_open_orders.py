"""
src/ingestion/ingest_open_orders.py

Stage: ingest the Open Orders sheet from the raw legacy ERP Excel
export into a tagged CSV. Lives in the open_orders track, not shared —
a failure here stops only the open_orders track; order_history and
inventory are unaffected.

Split out from the original excel_to_csv_pipeline.py (see
ingest_order_history.py's docstring for why). This script reads the
same physical workbook as ingest_order_history.py, independently: same
naming convention, own manifest, own metadata, so a broken Order
History sheet has no effect on whether Open Orders can ingest, and
vice versa.

Output contract (unchanged from the original combined version):
    data/raw_csv/open_orders_<batch_id>.csv
    data/metadata/batch_metadata_open_orders_<batch_id>.json
"""
...
OUTPUT_NAME = "open_orders"

MANIFEST_FILE = METADATA_DIR / "ingestion_manifest_open_orders.json"
...
logger = logging.getLogger("ingest_open_orders")
...
def find_input_file(explicit_path: str | None) -> Path:
    ...
    convention already in use for this source (same workbook
    ingest_order_history.py reads — Open Orders is a second sheet in
    it, not a separate file).
    """
    if explicit_path:
...
def check_already_ingested(source_hash: str, manifest: list[dict], force: bool) -> None:
    ...
    only (ingestion_manifest_open_orders.json) — see ingest_order_history.py
    for why the two ingestion stages don't share one manifest.
    """
    prior = [m for m in manifest if m["source_file_hash"] == source_hash]
    if prior and not force:
        prior_batches = [m["batch_id"] for m in prior]
        raise RuntimeError(
            f"This exact source file has already been ingested for Open "
            f"Orders (batch_id(s): {prior_batches}). Refusing to re-ingest "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Ingest the Open Orders sheet from the legacy ERP Excel export.")
    ...
    try:
        ...
        metadata_file = METADATA_DIR / f"batch_metadata_open_orders_{batch_id}.json"
        ...
        logger.exception("Open Orders ingestion failed")
        return 1
